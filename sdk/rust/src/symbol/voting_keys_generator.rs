...
impl<KG: KeyGenerator> VotingKeysGenerator<KG> {
    ...
    pub fn generate(&mut self, start_epoch: u64, end_epoch: u64) -> Result<Vec<u8>, SymbolError> {
        ...
        buffer.write_u64::<LittleEndian>(start_epoch)?; // 0..8   // start key identifier
        buffer.write_u64::<LittleEndian>(end_epoch)?; // 8..16  // end key identifier
        buffer.write_u64::<LittleEndian>(0xFFFFFFFFFFFFFFFF)?; // 16..24 // reserved - last (used) key identifier
        buffer.write_u64::<LittleEndian>(0xFFFFFFFFFFFFFFFF)?; // 24..32 // reserved - last wiped key identifier
        buffer.extend_from_slice(&self.root_private_key.pubilc_key().to_bytes()); // 32..64 // root voting public key
        buffer.write_u64::<LittleEndian>(start_epoch)?; // 64..72 // level 1/1 start key identifier
        buffer.write_u64::<LittleEndian>(end_epoch)?; // 72..80 // level 1/1 end key identifier
        ...
        for i in 0..num_epochs {
            ...
            parent_signed_payload_buffer.write_u64::<LittleEndian>(identifier)?; // 32..40
